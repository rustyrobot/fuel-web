...
# Configure alembic config for core db migrations
...
ALEMBIC_CONFIG.set_main_option('version_table', 'volume_manager_1_0_0_alembic_version')
...
def do_upgrade_head(config=None):
    config = config or ALEMBIC_CONFIG
    do_alembic_command("upgrade", config, "head")


def action_migrate_alembic(params, alembic_config=None):
    global ALEMBIC_CONFIG
    config = alembic_config or ALEMBIC_CONFIG
