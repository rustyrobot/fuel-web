...
class BaseChecker(object):
    ...
    def check_if_port_open(self, host, port):
        try:
...
class FuelUpgradeVerify(object):
    ...
    def verify(self):
        def check():
            not_running_services = self.get_not_running_services()
            if not_running_services:
                logger.info(
                    'Checkers which are still fail %s', not_running_services)
                return False
            return True
        ...
        try:
            ...
            raise errors.UpgradeVerificationError(
                'Failed to run services {0}'.format(
                    self.get_not_running_services()))

    def get_not_running_services(self):
        return list(set(self.expected_services) -
                    set(self.get_running_services()))
