    ...
    def test_deployment_task_managers(self):
        self.env.create(
            nodes_kwargs=[
                ...
                {"pending_deletion": True, 'status': NODE_STATUSES.provisioned},
            ]
        )
        supertask = self.env.launch_deployment()
        self.env.refresh_nodes()
        self.assertEqual(supertask.name, TASK_NAMES.deploy)
        self.assertIn(supertask.status, (TASK_STATUSES.running, TASK_STATUSES.ready))
        ...
        self.env.wait_for_nodes_status([self.env.nodes[0]], NODE_STATUSES.provisioning)
        self.env.wait_ready(
            supertask,
            60,
        ...
        self.env.wait_for_nodes_status([self.env.nodes[0]], NODE_STATUSES.ready)
        self.env.refresh_nodes()
        for n in filter(
